...
# Apostolic Fathers
# retrieve directly from git to ensure we get the latest
github_raw_af = 'https://raw.githubusercontent.com/jtauber/apostolic-fathers/master/texts/'
af_files = get_af_files()
...
af_counts = {'total': 0, 'tagged': 0, 'untagged': 0, 'latin': 0}
missed_words = {}

for af_filename in af_files:
    ...
    # open the file as a list of lines
    # open the file as a list of lines
    af_response = requests.get(github_raw_af + af_filename)
    lines = af_response.text.split('\n')
    for line in lines:
